...
export class NotificationService {
    ...
    async subscribe(userId: string, categories: string[]) {
        if (categories.includes('all')) {
            categories = ['new_friend_request', 'new_broadcast_message', 'accept_friend_request', 'other_categories'];
        }
    ...
    async addPushSubscription(userId: string, subscription: PushSubscription) {
        const query = 'INSERT INTO push_subscriptions (user_id, subscription) VALUES ($1, $2) ON CONFLICT (user_id) DO UPDATE SET subscription = $2';
    ...
    async sendNotificationOnDemand(senderId: string, receiverUserId: string, category: string, message: string) {
        if (category === 'broadcast') {
            ...
            for (const userId of userIds) {
                ...
                this.sendNotification(userId, category, { senderId, message, category });
                await this.sendPushNotification(userId, { senderId, message, category });
            }
        } else {
            await this.storeNotification(senderId, receiverUserId, category, message, 'unread');
            this.sendNotification(receiverUserId, category, { senderId, receiverUserId, message, category });
            await this.sendPushNotification(receiverUserId, { senderId, receiverUserId, message, category });
        }
    }
    ...
    async handleMockSQSMessage(message: Message) {
        ...
        if (category === 'broadcast') {
            ...
            for (const userId of userIds) {
                ...
                this.sendNotification(userId, category, { senderId, message: msg, category });
                await this.sendPushNotification(userId, { senderId, message: msg, category });
            }
        } else {
            await this.storeNotification(senderId, receiverUserId!, category, msg, 'unread'); // Non-null assertion since receiverUserId should be present
            this.sendNotification(receiverUserId!, category, { senderId, receiverUserId, message: msg, category });
